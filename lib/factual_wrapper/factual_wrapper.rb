...
module FactualWrapper
  ...
  def get_product
    ...
    product_info = []
    product_info.push(@factual.table("products-cpg-nutrition").search("Triscuit").rows[0]["brand"])
    product_info.push(@factual.table("products-cpg-nutrition").search("Triscuit").rows[0]["product_name"])
    product_info.push(@factual.table("products-cpg-nutrition").search("Triscuit").rows[0]["ingredients"])
  end
